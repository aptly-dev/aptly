...
func (s *PackageListSuite) TestFilter(c *C) {
	c.Check(func() { s.list.Filter([]PackageQuery{&PkgQuery{"abcd", "0.3", "i386"}}, false, false, nil, 0, nil) }, Panics, "list not indexed, can't filter")
	...
	result, err := s.il.Filter([]PackageQuery{&PkgQuery{"app", "1.1~bp1", "i386"}}, false, false, nil, 0, nil)
	c.Check(err, IsNil)
	...
	result, err = s.il.Filter([]PackageQuery{&PkgQuery{"app", "1.1~bp1", "i386"}, &PkgQuery{"dpkg", "1.7", "source"},
		&PkgQuery{"dpkg", "1.8", "amd64"}}, false, false, nil, 0, nil)
	c.Check(err, IsNil)
	...
	result, err = s.il.Filter([]PackageQuery{
		...
		&DependencyQuery{Dep: Dependency{Pkg: "aa", Relation: VersionGreater, Version: "3.0"}}}, false, false, nil, 0, nil)
	c.Check(err, IsNil)
	...
	result, err = s.il.Filter([]PackageQuery{&DependencyQuery{Dep: Dependency{Pkg: "app", Architecture: "i386"}}}, true, false, NewPackageList(), 0, []string{"i386"})
	c.Check(err, IsNil)
	...
	result, err = s.il.Filter([]PackageQuery{
		...
		&DependencyQuery{Dep: Dependency{Pkg: "data"}}}, true, false, NewPackageList(), 0, []string{"i386", "amd64"})
	c.Check(err, IsNil)
	...
	result, err = s.il.Filter([]PackageQuery{&OrQuery{&PkgQuery{"app", "1.1~bp1", "i386"},
		&DependencyQuery{Dep: Dependency{Pkg: "dpkg", Relation: VersionGreater, Version: "1.6.1-3"}}}}, false, false, nil, 0, nil)
	c.Check(err, IsNil)
	...
	result, err = s.il.Filter([]PackageQuery{&OrQuery{&PkgQuery{"app", "1.1~bp1", "i386"},
		&FieldQuery{Field: "$Architecture", Relation: VersionEqual, Value: "s390"}}}, false, false, nil, 0, nil)
	c.Check(err, IsNil)
	...
	result, err = s.il.Filter([]PackageQuery{&AndQuery{
		&FieldQuery{Field: "$Architecture", Relation: VersionPatternMatch, Value: "i*6"}, &PkgQuery{"app", "1.1~bp1", "i386"}}}, false, false, nil, 0, nil)
	c.Check(err, IsNil)
	c.Check(plString(result), Equals, "app_1.1~bp1_i386")

	result, err = s.il.Filter([]PackageQuery{&NotQuery{
		&FieldQuery{Field: "$Architecture", Relation: VersionPatternMatch, Value: "i*6"}}}, false, false, nil, 0, nil)
	c.Check(err, IsNil)
	...
	result, err = s.il.Filter([]PackageQuery{&AndQuery{
		&FieldQuery{Field: "$Architecture", Relation: VersionRegexp, Value: "i.*6", Regexp: regexp.MustCompile("i.*6")}, &PkgQuery{"app", "1.1~bp1", "i386"}}}, false, false, nil, 0, nil)
	c.Check(err, IsNil)
	c.Check(plString(result), Equals, "app_1.1~bp1_i386")
