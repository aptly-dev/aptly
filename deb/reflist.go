...
func bucketRefPrefix(ref []byte) []byte {
	...
	libPrefix := []byte("lib")
	ref = bytes.TrimPrefix(ref, libPrefix)
...
// NewSplitRefListFromPackageList creates SplitRefList from PackageList
...
// Replace removes all the refs inside and replaces them with those in the given
// reflist
...
func (sl *SplitRefList) FilterLatestRefs() {
	for idx, bucket := range sl.bucketRefs {
		if bucket != nil {
			clone := bucket.Clone()
			clone.FilterLatestRefs()
			sl.Buckets[idx] = reflistDigest(clone)
			sl.bucketRefs[idx] = clone
		}
	}
}
...
func (sl *SplitRefList) Flatten() *PackageRefList {
	...
	_ = sl.ForEach(func(ref []byte) error {
	...
	for idx := range sl.Buckets {
		...
		if digest := sl.Buckets[idx]; len(digest) > 0 {
			...
			if bucket == nil {
				bucket = NewPackageRefList()
				prefix := segmentPrefix(bucketDigestEncoding.EncodeToString(digest))
				err := collection.db.ProcessByPrefix(prefix, func(key []byte, value []byte) error {
...
func clearSegmentRefs(reflist *PackageRefList, recommendedMaxKVSize int) {
	// 70 is a rough average for the length of a full package reference.
...
func (collection *RefListCollection) updateWithContext(sl *SplitRefList, key []byte, uctx *reflistUpdateContext) error {
	if sl != nil {
		...
		for idx, digest := range sl.Buckets {
			...
			// that limit.) Then, we also have a 5-byte "Refs" string and the
			// single-byte outer map header.
			initialSize := len(segmentIndexKey(prefix, 0)) + 3 + 5 + 1
			size := initialSize
			segment := 0
...
type RefListMigrationOptions struct {
	DryRun bool
	Revert bool
}

// NewMigration creates an empty RefListMigration
func (collection *RefListCollection) NewMigration(options RefListMigrationOptions) *RefListMigration {
...
func (migration *RefListMigration) Flush() error {
	if migration.batchStats.Reflists > 0 {
		if !migration.dryRun {
...
func (collection *RefListCollection) LoadCompleteAndMigrate(sl *SplitRefList, key []byte, migration *RefListMigration) error {
	...
	case reflistStorageFormatInline:
		...
		err := collection.updateWithContext(sl, key, &reflistUpdateContext{
			rb:    migration.rb,
	...
	case reflistStorageFormatSplit:
		if err := collection.loadBuckets(sl); err != nil {
			return err
		}
		...
		if migration.rb == nil {
			// Skip setting alreadyWritten since that's only useful for migrating *to*
			// split reflists.
			migration.rb = &RefListBatch{batch: collection.db.CreateBatch()}
		}

		reflist := sl.Flatten()
		if err := migration.rb.batch.Put(key, reflist.Encode()); err != nil {
			return err
		}
		migration.batchStats.Reflists += 1

		if migration.batchStats.Reflists > maxMigratorBatch {
			if err := migration.Flush(); err != nil {
				return err
			}
		}
...
type AnyRefList interface {
	Has(p *Package) bool
	Len() int                                 // safe to call for 'nil'
	ForEach(handler func([]byte) error) error // safe to call for 'nil'
	FilterLatestRefs()
}
