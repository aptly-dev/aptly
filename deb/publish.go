	...
	if revision == nil {
		...
		if p.SourceKind == SourceLocalRepo {
			...
			for component, name := range revision.Sources {
				...
				err = localRepoCollection.LoadComplete(localRepo, collectionFactory.RefListCollection())
				if err != nil {
					return result, fmt.Errorf("unable to update: %s", err)
				}
...
func (collection *PublishedRepoCollection) Update(repo *PublishedRepo, reflistCollection *RefListCollection) error {
	...
	if repo.SourceKind == SourceLocalRepo {
		...
		for component, item := range repo.sourceItems {
			err = reflistCollection.UpdateInBatch(item.packageRefs, repo.RefKey(component), rb)
			if err != nil {
				return err
			}
		}
