...
func (s *PackageRefListSuite) TestForEachNilList(c *C) {
	...
	var sl *SplitRefList
	called = false
	err = sl.ForEach(func([]byte) error {
		called = true
		return nil
	})
	c.Assert(err, IsNil)
...
func (s *PackageRefListSuite) TestRefListCollectionLoadSave(c *C) {
	...
	_ = packageCollection.Update(s.p1)
	_ = packageCollection.Update(s.p2)
	_ = packageCollection.Update(s.p3)
	_ = packageCollection.Update(s.p4)
	_ = packageCollection.Update(s.p5)
	_ = packageCollection.Update(s.p6)
...
func (s *PackageRefListSuite) TestRefListCollectionMigrate(c *C) {
	...
	// Test the same inner reflist contents in two places.
	keys := [][]byte{[]byte("test"), []byte("test2")}
	...
	for i, key := range keys {
		...
		migrator := reflistCollection.NewMigration(RefListMigrationOptions{})
		...
		if i == 0 {
			// First iteration, non-zero buckets should have been saved.
			c.Check(stats.Buckets, Not(Equals), 0)
			c.Check(stats.Segments, Not(Equals), 0)
		} else {
			// Second iteration, reusing existing buckets so no new ones.
			c.Check(stats.Buckets, Equals, 0)
			c.Check(stats.Segments, Equals, 0)
		}
	...
	// Now revert the split reflist migration.
	migrator := reflistCollection.NewMigration(RefListMigrationOptions{Revert: true})
	for i, key := range keys {
		sl := NewSplitRefList()
		...
		stats := migrator.Stats()
		c.Check(stats.Reflists, Equals, i+1)
		c.Check(stats.Buckets, Equals, 0)
		c.Check(stats.Segments, Equals, 0)
	}
	...
	for _, key := range keys {
		...
		err = reflist.Decode(value)
		c.Assert(err, IsNil)
		c.Check(toStrSlice(reflist), DeepEquals, toStrSlice(NewPackageRefListFromPackageList(list)))
	}
}
